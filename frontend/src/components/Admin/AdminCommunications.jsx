...
const AdminCommunications = () => {
  ...
  const navItems = [
    ...
    { to: "/admincoms", icon: <FaEnvelope />, label: "Communications", active: true },
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post("/admin/communications", fd, {
  ...
  const fetchBlobAndAct = async (id, index, action) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/admin/communications/${id}/${endpoint}/${index}`, {
        method: "GET",
