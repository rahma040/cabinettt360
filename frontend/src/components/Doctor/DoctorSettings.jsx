...
const G = `
  * { font-family: 'Sora', sans-serif; box-sizing: border-box; }
  ...
  .btn-accent:disabled { opacity:.5; transform:none; box-shadow:none; cursor:not-allowed; }
  ...
  .st-input:focus { outline:none; border-color:var(--accent); box-shadow:0 0 0 3px rgba(59,126,248,.2); }
...
const DoctorSettings = () => {
  ...
  const navItems = [
    ...
    { to: "/doctuto", icon: <FaUser />, label: "Tutoriel" },
    { to: "/docsettings", icon: <FaCog />, label: "Paramètres", active: true },
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      const msg = err.response?.data?.error || err.response?.data?.errors || "Erreur lors de la mise à jour du profil.";
      addToast(typeof msg === 'string' ? msg : "Erreur de validation", "error");
      setError(msg);
    } finally {
      setSaving(false);
    }
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-8 gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl" style={{ background: "#fff", border: "1.5px solid var(--border)" }}>
                <div className="w-7 h-7 rounded-lg flex items-center justify-center text-white text-xs db-logo-ring" style={{ fontWeight: 700 }}>
                  {user?.name?.charAt(0) || "D"}
                </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex justify-end pt-2">
                <button
                  type="submit"
                  disabled={saving}
                  className="btn-accent px-6 py-3 rounded-xl text-sm flex items-center gap-2"
