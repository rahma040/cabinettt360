    ...
    try {
      ...
      if (err.response) {
        if (err.response.status === 403 && err.response.data.error && 
            err.response.data.error.includes("attente de vérification")) {
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl shadow-2xl p-8 border border-white/20">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="animate-slideInLeft animation-delay-100">
              ...
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <FiLock className="text-gray-400 group-focus-within:text-blue-400 transition-colors" />
                </div>
                <input
            ...
            <div className="flex items-center justify-between animate-fadeIn animation-delay-200">
              <label className="flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  checked={rememberMe}
                  onChange={(e) => setRememberMe(e.target.checked)}
                  className="w-4 h-4 bg-white/5 border border-white/10 rounded focus:ring-blue-500 focus:ring-2"
                />
                <span className="ml-2 text-sm text-gray-300">Se souvenir de moi</span>
              </label>
              <Link to="/passreset" className="text-sm text-blue-400 hover:text-blue-300 transition-colors">
                Mot de passe oublié ?
              </Link>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="relative w-full py-3 px-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed overflow-hidden group transform hover:scale-[1.02] active:scale-[0.98]"
            >
          ...
          <p className="mt-6 text-center text-gray-400 animate-fadeIn animation-delay-300">
